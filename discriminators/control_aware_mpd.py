...
class ControlAwareDiscriminatorP(nn.Module):
    ...
    def __init__(self,
        ...
        self.convs.append(nn.Conv1d(1 + control_dim, 32, kernel_size, 1, padding=2))
        ...
        # Control projection: maps control parameters to the same temporal resolution
        # We'll use a simple linear projection per time step
        self.control_proj = nn.Linear(control_dim, control_dim)
    ...
    def forward(self,
        ...
        if control is not None:
            ...
            # Project control
            control = self.control_proj(control.transpose(1, 2)).transpose(1, 2)
        ...
        # Actually, standard MPD: reshape to (B, 1, T//period, period) -> (B, period, T//period)
        # But we have multiple channels now.
        # We'll do: (B, C, T) -> (B, C, T//period, period) -> (B, C*period, T//period)
        b, c, t = x.shape
        if t % self.period != 0:
